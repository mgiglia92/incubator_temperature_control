...
void TempControlPan::initialize()
{
...
double TempControlPan::convert_to_C(int raw, double rn, double rs, double b)
{
...
void TempControlPan::update_temps()
{
...
void TempControlPan::check_safety()
{
    control_data.error_type = 0;
    
    //Limit T incubator reference (0 - 50 allowed)
    if(control_data.T_incubator_ref > 120.0) { control_data.T_incubator_ref = 120.0; control_data.error_type = 1;}
...
//A3
    ...
    if(control_data.error_type != 0) { control_data.H0 = 0; control_data.H1 = 0;}

    return control_data.error_type;
}

void TempControlPan::do_controls()
{    
    if(check_dt())
    {
        //Get readings
        ...
        double Tavg = (control_data.T2 + control_data.T3)/2.0; //Get average between two thermistor inside pan
        ...
        double heater_output_0 = PID(control_data.T_heater_ref, control_data.T0);
        double heater_output_1 = PID(control_data.T_heater_ref, control_data.T1);
        ...
        control_data.H0 = max(min(heater_output_0, 225), 0);
        control_data.H1 = max(min(heater_output_1, 225), 0);
...
void TempControlPan::do_debug_mode(SerialComms *comms)
{
    // update temps()
    if(check_dt())
    {
...
void TempControlPan::init_control_data(ControlData data)
{
    control_data = data;
}

bool TempControlPan::check_dt()
{
    current = micros();
